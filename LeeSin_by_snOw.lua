...
local SCRIPT_NAME = "LeeSin_by_snOw"
...
function OnLoad()
	HConfig = scriptConfig("Lee Sin by snOwPGC", "LeeSin")
	...
	HConfig.draws:addParam("drawQ", "Draw Q Range", SCRIPT_PARAM_ONOFF, true)
	--Start Snow
	HConfig.draws:addParam("drawW", "Draw W Range", SCRIPT_PARAM_ONOFF, true)
	HConfig.draws:addParam("drawE", "Draw E Range", SCRIPT_PARAM_ONOFF, true)
	--End Snow
	...
	HConfig.miscs:addParam("predInSec", "Use prediction for Insec", SCRIPT_PARAM_ONOFF, true)
	...
	HConfig:addTS(ts)
	--KS BY SNOW
	HConfig:addSubMenu("Kill Steal", "ks")
	HConfig.ks:addParam("ks", "Enable this for Kill Steal work", SCRIPT_PARAM_ONOFF, true)
	HConfig.ks:addParam("ksq", "KS with Q", SCRIPT_PARAM_ONOFF, true)
	HConfig.ks:addParam("kse", "KS with E", SCRIPT_PARAM_ONOFF, true)
	HConfig.ks:addParam("ksr", "KS with R", SCRIPT_PARAM_ONOFF, true)
	HConfig.ks:addParam("ksi", "KS with Ignite", SCRIPT_PARAM_ONOFF, true)
	--end ks by Snow
	
	VP = VPrediction()
	--SOW
	OW = SOW(VP)
	HConfig:addSubMenu("Orbwalking", "Orbwalking")
	OW:LoadToMenu(HConfig.Orbwalking)
	--End
	...
	PrintChat(" >> Lee Sin by snOwPGC ".."v"..version)
end
...
function KS()
	...
	local igniteReady = (ignite ~= nil and myHero:CanUseSpell(ignite) == READY)
	local q1 = (myHero:GetSpellData(_Q).name == "BlindMonkQOne")
	
			for i, enemy in pairs(EnemyTB) do
				if ValidTarget(enemy) then
					if HConfig.ks.ksq and QREADY and enemy.health < getDmg("Q", enemy, MyHero) and GetDistance(enemy, myHero) < 1050 and q1 then
					...
					if HConfig.ks.kse and EREADY and enemy.health < getDmg("E", enemy, myHero) and GetDistance(enemy, myHero) < 450 then
					CastSpell(_E)
					end
					
					if HConfig.ks.ksr and enemy.health < getDmg("R", enemy, myHero) and GetDistance(enemy, myHero) < 300 then
					CastSpell(_R, enemy)
					end
					if ignite ~= nil then
						if igniteReady and HConfig.ks.ksi and enemyhealth < getDmg("IGNITE", enemy, myHero) and GetDistance(enemy, myHero) < 550 then
...
--Start Snow
	...
	if HConfig.draws.drawW then
		if myHero:CanUseSpell(_W) == READY then
			DrawCircle(myHero.x, myHero.y , myHero.z, 700, 0x26CFDF)
		end
	end
	if HConfig.draws.drawE then
		if myHero:CanUseSpell(_E) == READY then
			DrawCircle(myHero.x, myHero.y, myHero.z, 450, 0xF61B0B)
		end
	end
	--End Snow
